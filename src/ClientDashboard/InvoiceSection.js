...
export default function InvoiceSection({ client, setClient }) {
  ...
  const handleSaveInvoiceAndInitialPayment = async () => {
    ...
    const newPayment = {
      ...
      isInitialPayment: true // Marcamos este pago como inicial
    };
  ...
  const handleDeleteInvoice = async () => {
    ...
    // Filtrar pagos para remover el pago inicial asociado con este invoice
    const updatedPayments = (client.payments || []).filter(p => 
      !(p.date === client.initialPaymentDate && p.isInitialPayment)
    );
    ...
    await updateDoc(clientRef, {
      ...
      payments: updatedPayments // Actualizamos la lista de pagos
    });
  ...
  const handleEditClick = () => {
    if (client.invoiceTotal) {
      setInvoiceInput(client.invoiceTotal.toString());
      
      // Buscar el pago inicial para pre-llenar el monto
      const initialPayment = (client.payments || []).find(p => 
        p.date === client.initialPaymentDate && p.isInitialPayment
      );
      
      if (initialPayment) {
        setInitialPaymentInput(initialPayment.amount.toString());
      }
      
      setInitialPaymentDate(client.initialPaymentDate || '');
      setFirstInstallmentDate(client.firstInstallmentDate || '');
      setEditMode(true);
    }
  };
